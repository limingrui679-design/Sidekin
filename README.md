...
当前版本：`1.2.0`（Build 3），Apple Silicon `arm64`，macOS 14+。
...
当前共有 26 项本地自检、4 项 API 模拟自检。它们覆盖成长阈值、1–8 阶段规划、模板管理、整套与单阶段断点恢复、质量费用、抠图颜色保护、50 张透明资源、Codex 事件隐私过滤、Hooks 安全，以及生成/编辑端点的模拟请求与响应。模拟自检不会读取真实 Key，也不会产生 API 费用。
