...
ARCHIVE="$OUTPUT_DIR/CainiaoPet-GitHub-Source.zip"
CHECKSUM="$ARCHIVE.sha256"

mkdir -p "$OUTPUT_DIR"
PACKAGE_TEMP="$(mktemp -d /tmp/cainiao-pet-source.XXXXXX)"
cleanup() {
  case "$PACKAGE_TEMP" in
    /tmp/cainiao-pet-source.*) rm -rf -- "$PACKAGE_TEMP" ;;
  esac
}
trap cleanup EXIT

STAGING="$PACKAGE_TEMP/CainiaoPet"
mkdir -p "$STAGING"
...
swift "$STAGING/Scripts/verify-character-assets.swift" \
  "$STAGING/Sources/CainiaoPetApp/Resources/Characters"

rm -f -- "$ARCHIVE"
(
  cd "$PACKAGE_TEMP"
  COPYFILE_DISABLE=1 /usr/bin/zip -qry -X "$ARCHIVE" CainiaoPet \
...
print -r -- "$ZIP_LIST" | /usr/bin/grep -qx 'CainiaoPet/Package.swift'
print -r -- "$ZIP_LIST" | /usr/bin/grep -qx 'CainiaoPet/ArtSources/PET_THEME_CATALOG.json'
print -r -- "$ZIP_LIST" | /usr/bin/grep -qx 'CainiaoPet/docs/LINEAGE_AUDIT.md'

RESOURCE_COUNT="$(print -r -- "$ZIP_LIST" | /usr/bin/grep -Ec \
  '^CainiaoPet/Sources/CainiaoPetApp/Resources/Characters/[^/]+\.png$')"
PROMPT_COUNT="$(print -r -- "$ZIP_LIST" | /usr/bin/grep -Ec \
  '^CainiaoPet/ArtSources/Prompts/[^/]+/[^/]+\.txt$')"
AUDIT_SHEET_COUNT="$(print -r -- "$ZIP_LIST" | /usr/bin/grep -Ec \
  '^CainiaoPet/ArtSources/AuditSheets/lineage-audit-[0-9]{2}\.png$')"
[[ "$RESOURCE_COUNT" == "500" ]] || {
