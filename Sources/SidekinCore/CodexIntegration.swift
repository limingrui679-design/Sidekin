...
public struct CodexHooksInstaller: Sendable {
    public static let commandMarker = "SidekinBridge"
    public static let legacyCommandMarker = "CainiaoPetBridge"
    ...
    public func install(at hooksURL: URL, bridgeExecutable: URL) throws {
        ...
        if root["description"] == nil {
            root["description"] = "Local Codex lifecycle hooks. Sidekin entries are added only after user confirmation."
        }
        ...
        return groups.compactMap { group in
            ...
            let retained = handlers.filter { handler in
                ...
                return !command.contains(Self.commandMarker)
                    && !command.contains(Self.legacyCommandMarker)
            }
