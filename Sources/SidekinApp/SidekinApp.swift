...
@main
struct SidekinApp: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) private var appDelegate
    @StateObject private var model = AppModel.shared

    var body: some Scene {
        Window("Sidekin Command Center", id: "control-center") {
        ...
        MenuBarExtra("Sidekin", systemImage: "sparkles") {
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    static func showControlCenter() {
        ...
        let candidate = NSApplication.shared.windows.first {
            !($0 is NSPanel) && $0.title == "Sidekin Command Center"
