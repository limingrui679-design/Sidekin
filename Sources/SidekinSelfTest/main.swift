...
test("Installing Codex hooks preserves the user's existing configuration") {
    ...
    let bridgeURL = directory.appendingPathComponent("SidekinBridge")
    ...
    try expect(text.contains("SidekinBridge"), "The Sidekin hook was not written")
    ...
    try expect(!text.contains("SidekinBridge"), "The Sidekin hook remains after uninstallation")
}

test("The Sidekin rename migrates legacy local data without overwriting new data") {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent(UUID().uuidString, isDirectory: true)
    let legacyDirectory = directory
        .appendingPathComponent("Library/Application Support/CainiaoPet", isDirectory: true)
    let sidekinDirectory = directory
        .appendingPathComponent("Library/Application Support/Sidekin", isDirectory: true)
    defer { try? FileManager.default.removeItem(at: directory) }

    try FileManager.default.createDirectory(at: legacyDirectory, withIntermediateDirectories: true)
    try Data("legacy-save".utf8).write(
        to: legacyDirectory.appendingPathComponent("pet-state.json"),
        options: .atomic
    )

    let fileManager = FileManager()
    let migrated = SidekinPaths.prepareApplicationSupportDirectory(
        in: directory.appendingPathComponent("Library/Application Support", isDirectory: true),
        fileManager: fileManager
    )
    let migratedData = try Data(
        contentsOf: sidekinDirectory.appendingPathComponent("pet-state.json")
    )
    try expect(
        String(data: migratedData, encoding: .utf8) == "legacy-save",
        "The legacy save was not preserved during migration"
    )
    try expect(migrated.lastPathComponent == "Sidekin", "The new application-support directory is incorrect")
}
